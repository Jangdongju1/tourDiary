...
let data = document.getElementById("encodedData").value;
let jsonData = JSON.parse(decodeURIComponent(escape(atob(data))));
...
let positions = [];    // 비어있는 배열을 선언하고 

for(let i =0 ; i<jsonData.length; i++){  //객체를 변수로 선언할 때는 쉼표 없이도 객체를 선언할 수 있다. 
	let info = {     // 객채를 차례로 생성해서 
...
for(let i =0 ; i<jsonData.length; i++){
	let info  = new kakao.maps.LatLng(jsonData[i].latitude, jsonData[i].longitude);	// 카카오에서 제공하는 객체로 좌표정보가 담긴 객체를 생성해서 배열에 저장한다. 
	
	spot.push(info);   // 좌표로 이루어진 객체의 배열을 생성
}


let polyline = new kakao.maps.Polyline({  // 지도에 선을 긋는 객체를 생성한다.  매개변수는 객체로 지도와 경로(좌표값) 그리고 다양한 속성들로 구성된다.
    map: map,
    path: spot,
...
for(let i = 0 ; i<length ; i++){  // 경로의 길이 만큼 돌면서 xy좌표값을 객체 배열화한다.
	let latInfo = {
...
if (coord.length > 1) {
  ...
  .then(results => {
    ...
    let distanceObj = document.querySelectorAll(".style2 > div:nth-child(2)");
